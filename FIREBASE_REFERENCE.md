...
| Sign-in method enabled | Email/Password + Google + **Phone (SMS one-time codes)** - verified 2026-09-08/09 via the Identity Toolkit management API. Phone was switched on on 2026-09-08 for the checkout WhatsApp-number verification (no code, no SMS template change needed - Firebase sends `%LOGIN_CODE% is your verification code for %APP_NAME%.`); email + Google were already on and are untouched. Since 2026-09-09 the app ALSO offers **Continue with Twitter/X** and **Continue with Yahoo** on the account page, but those two are NOT yet switched on in the console - until the owner adds each provider under Authentication > Sign-in method (Twitter needs an X developer app + API key/secret; Yahoo needs a Yahoo developer app), the buttons answer with a friendly "not switched on yet" message |
...
| `orders` | owner may read orders linked to their own account (`customerId` == their uid); create ONLY the "captured order" allowlist (checkout fallback); never delete. Since 2026-09-08 a captured order's `phone` must be the **canonical `+91XXXXXXXXXX`** number and proven server-side (`phoneVerified: true` in the order, then the rules' `phoneProofHolds`): either on the caller's own Firebase auth token (`phone_number` claim - Firebase only adds that claim after it verified the one-time code itself), or an admin attestation on the caller's own `customers/{uid}` document (`phone` == the order's phone and `phoneVerified: true` - those fields are admin-only). Since 2026-09-09, while the published temporary-code flag is on (`siteConfig/public.whatsappCodeFallback == true`, written by the owner via admin SDK), the rules AND the trusted backend also accept the shared code **123456** entered in the checkout slot - no SMS is sent in that mode (no SMS subscription yet). The browser can never write an unverified number or forge the marker | read; update status-only |
...
| Field | Live value (2026-09-09) |
| --- | --- |
| `whatsappNumber` | `916363816465` |
| `deliveryFee` | `39` |
| `deliveryEnabled` | `true` |
| `whatsappCodeFallback` | `true` (2026-09-09 - the published switch behind the temporary 6-digit code 123456; see the orders section below) |
| `currency` | `INR` |
...
security rules only accept that when the number is proven server-side: the
`phone_number` claim on the caller's own Firebase auth token (Firebase only adds that
claim after it verified the one-time code itself), or an admin attestation on the
caller's own `customers/{uid}` document, or - since 2026-09-09 - while the published
temporary-code flag (`siteConfig/public.whatsappCodeFallback`) is on, the temporary
shared code **123456** entered in the checkout slot instead of an SMS code. Older
orders keep whatever format they were saved in and are unaffected; admins never see
OTPs or tokens (none exist - codes go straight from the customer's browser to
Firebase, and 123456 is a published shop-wide temporary code, verified only while the
flag is on).
...
| --- | --- |
| `firestore.rules` | Security rules for every collection (the schema tables above come from here). Since 2026-09-08 the orders `create` gate proves the canonical phone server-side (`phoneProofHolds`): the caller's auth-token `phone_number` claim, or an admin attestation on the caller's own `customers/{uid}` profile; since 2026-09-09 the gate is `phoneProofOrFallbackHolds` - the same proof OR the published temporary-code flag (`siteConfig/public.whatsappCodeFallback == true`) |
...
| `functions/index.js` + `functions/test/` | Trusted order backend + tests (optional; not deployed on the free plan). Mirrors the phone proof inside its transaction - attestation and the temporary-code fallback included (29 tests pass) |
...
3. Re-create the settings + admin grant (console or a script using the admin key):
   ...
   - make sure Authentication > Email/Password is enabled and the admin user exists
     (Google/Twitter/Yahoo buttons in the app are optional extras - each shows a
     friendly "not switched on yet" message until enabled under Sign-in method),
